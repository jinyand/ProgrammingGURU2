...
public class TabLostFragment extends Fragment {
    ...
    public void searchList(){
        ...
        List<LostBean> tempList = new ArrayList<LostBean>();
        List<LostBean> allList = mSaveBean.getLostBeanList();

        // 멤버리스트를 돌면서 하나씩 비교검색한다.
        for (int i=0; i<allList.size(); i++){
            LostBean bean = allList.get(i);
        ...
        if (search != null && search.length() > 0 && tempList.size() > 0){
            mAdapter = new LostAdapter(getActivity(), tempList);
        }
        ...
        // 입력한 검색어가 없는 경우에는 원래의 리스트로 복원한다.
            ...
            mAdapter = new LostAdapter(getActivity(), allList);
        }

        mLstLostMember.setAdapter(mAdapter);
    ...
    public void onResume() {
        ...
        if(jsonStr != null) {
            ...
            if (mSaveBean != null) {
                mAdapter = new LostAdapter(getActivity(), mSaveBean.getLostBeanList());
                mLstLostMember.setAdapter(mAdapter);
            }
        }
